package com.hang.common.handle;

import com.hang.common.domain.FoFunction;
import com.hang.common.domain.FoManagerGroup;
import com.hang.common.domain.JsonResult;
import com.hang.common.exception.JsonException;
import com.hang.common.service.FoFunctionService;
import com.hang.common.service.FoManagerGroupService;
import com.hang.common.utils.JsonResultUtil;
import com.hang.common.utils.MyArrayUtil;
    ...
    public void adminLeftList(HttpServletRequest request, ModelMap map) throws Exception {
        ...
        if(uri.indexOf("/admin/") != -1){//如果为后台页面
            if(uri.indexOf("/admin/login") != -1 || uri.indexOf("/admin/kaptcha") != -1){
                ...
                if(foManagerGroup.getFunctioninfo().equals("CJ")){
                    //超级管理员不作处理
                    foFid = null;zoFid = null;
                }else{
                    //得到用户组权限的FIDs
                    String fidin = foManagerGroup.getFunctioninfo().substring(0,foManagerGroup.getFunctioninfo().length()-1);
